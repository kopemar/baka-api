...
#   end
#
#   test "Empty Shifts Fix the solution" do
#     20.times do
...
#     initial_violations = Scheduling::SpecializedPreferred.get_violation_hash(ShiftTemplate::in_scheduling_period(@period.id), schedule)
#     initial_sanction = initial_violations[:sanction]
#
#     Scheduling::Scheduling.new({ id: @period.id, priorities: { :no_empty_shifts => 10} }).call
#
#     schedule = get_period_as_schedule(@period)
#     violations = Scheduling::SpecializedPreferred.get_violation_hash(ShiftTemplate::in_scheduling_period(@period.id), schedule)
